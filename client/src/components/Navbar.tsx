        ...
        <div className="max-w-[1400px] mx-auto px-4 lg:px-20 flex justify-between items-center text-[13px] font-medium">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2.5 group cursor-pointer">
              <div className="bg-slate-300/60 dark:bg-white/10 p-1.5 rounded-md group-hover:bg-[#ce181e] group-hover:text-white transition-colors">
          ...
          {/* <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              {[Facebook, Instagram, Twitter, Linkedin].map((Icon, i) => (
                <a key={i} href="#" className="text-gray-400 hover:text-[#ce181e] transition-colors">
      ...
      <nav className="bg-white dark:bg-[#0f172a] shadow-lg relative h-14 lg:h-16 transition-all duration-300">
        <div className="max-w-[1400px] mx-auto h-full flex items-center justify-between relative px-4 lg:px-20">
            ...
            <div className="flex items-center space-x-1">
              {links.map((link) => (
                <Link key={link.href} href={link.href}>
                  <div className={`px-5 py-2 text-[15px] font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer relative group ${location === link.href ? "text-[#ce181e]" : "text-slate-800 dark:text-white"}`}>
                    {link.label}
                    <span className={`absolute bottom-0 left-5 right-5 h-0.5 bg-[#ce181e] transition-all duration-300 ${location === link.href ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`} />
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="flex items-center gap-4 lg:gap-6 relative z-10">
            ...
            <Link href="/apply">
              <Button className="bg-[#ce181e] hover:bg-[#a31317] text-white font-black uppercase tracking-widest px-6 h-10 rounded-md shadow-lg shadow-[#ce181e]/30 transition-all active:scale-95 flex items-center gap-2 group text-xs hover:shadow-xl hover:shadow-[#ce181e]/40 animate-float">
                {t.nav.applyNow}
              ...
              <div className="px-6 py-8 space-y-4">
                {links.map((link) => (
                  <Link key={link.href} href={link.href} onClick={() => setIsOpen(false)}>
                    <div className={`text-lg font-bold uppercase tracking-wide py-2 ${location === link.href ? "text-[#ce181e]" : "text-slate-800 dark:text-white"}`}>
                      {link.label}
                    </div>
                  </Link>
                ))}
                <div className="pt-6 border-t border-slate-100 dark:border-white/10 space-y-4">
                  <div className="flex items-center gap-4 text-sm text-slate-500">
                    <Phone className="w-4 h-4 text-[#ce181e]" /> (206) 274-8232
                  </div>
                  <Link href="/apply" onClick={() => setIsOpen(false)}>
                    <Button className="w-full bg-[#ce181e] hover:bg-[#b91c1c] text-white font-bold h-14 uppercase tracking-widest">
                      {t.nav.applyNow}
                    </Button>
                  </Link>
                </div>
